...
def execute_jmeter_task(self, task_id: str, jmx_file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        command_args = [
            "-n",  # non-GUI mode
            "-t", str(jmx_path),  # test plan
            "-l", str(output_path),  # log file
            "-j", str(settings.jtl_files_path / f"{jmx_path.stem}_{timestamp}.log"),  # jmeter log
...
def generate_html_report_task(self, task_id: str, jtl_file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        jmeter_cmd = f"{settings.jmeter_home}/bin/jmeter"
        command_args = [
            "-g", str(jtl_path),  # input JTL file
            "-o", str(report_dir)  # output report directory
        ]
