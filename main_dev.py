"""Development version of main application with SQLite."""
import os
import sys
...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting JMeter Toolkit (Development Mode)...")
...
app = FastAPI(
    ...
    description="A powerful toolkit for JMeter test management (Development Mode)",
    lifespan=lifespan
)
...
# Simplified execution endpoint (synchronous for development)
@app.post("/execute", response_model=APIResponse[TaskResponse])
async def execute_jmx_sync(
    file_name: str,
    jmeter_manager: JMeterManager = Depends(get_jmeter_manager)
):
    """Execute JMX file synchronously (development mode)."""
    try:
        # For development, use synchronous execution
        result = await jmeter_manager.execute_jmx_sync_dev(file_name)
        ...
        return APIResponse.success_response(
            data=response_data,
            message="JMeter execution completed"
        )
...
async def upload_and_execute(
    ...
    try:
        ...
        # Execute file synchronously for development
        execution_result = await jmeter_manager.execute_jmx_sync_dev(upload_result["file_name"])
